...
In `audio_dl.py`: `__version__ = "2.3.0"`. In `pyproject.toml`: `version = "2.3.0"`. (Dual-sourced by convention; `/release-helper` checks both.)
...
```markdown
## v2.3 — Related-content discovery ("More like this")
...
git commit -m "chore: v2.3.0 — related-content discovery release notes + docs"
```
