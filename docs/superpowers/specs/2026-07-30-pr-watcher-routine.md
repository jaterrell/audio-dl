...
10. **`create_or_update_file` on this doc is destructive if the `content`
    param isn't the FULL file** — it's a whole-file overwrite, not an
    append/patch. On 2026-08-06 (second invocation) a run briefly clobbered
    every section of this file except the Run log by passing only the Run
    log text as `content`; caught immediately (the tool response's returned
    file `size` was implausibly small for this doc) and corrected in the
    very next call by re-fetching + reconstructing the full file. Any future
    run editing this file via the GitHub MCP `create_or_update_file` tool
...
- **2026-08-06 (second invocation):** Tenth consecutive nothing-to-do run,
  ...
  — left alone. Mid-run correction: an earlier write to this same file in
  this invocation passed only the Run log section as `content` to
  `create_or_update_file`, which overwrote (not appended to) the whole
  file — wiping the "What it does," "Gaps found," and "Recommended routine
  edits" sections. Caught immediately from the implausibly small returned
  file `size`, and fixed in the very next call by re-fetching and pushing
  the complete reconstructed document. Added recommended-edit item 10 above
  so a future run doesn't repeat the mistake. No new information otherwise
  surfaced this run to add to "Recommended routine edits" — items 1-9
  remain the actionable, paste-ready backlog against the live routine
  prompt; confirmed once more (verified this run's task-description text
  directly) that none have landed yet: Step 4.7 still literally says "both
  repos" and doesn't enumerate project-tzu, Step 2's bucket triage still
  has no draft/stacked or `COMMENTED`-review carve-out, Step 3e's REPO
  CONVENTIONS is still audio-dl-only, and there's still no
  `SELF_LOGIN`/branch-pattern gate on auto-merge. Given ten straight
  identical outcomes, future steady-state runs should default to a short
...
- **2026-08-08:** Fifteenth consecutive nothing-to-do run, same steady
  state — 0 open PRs in audio-dl / audio-dl-internal, 0 untagged release
  commits (`v2.5.0` still matches the latest release-commit subject on
  `main` HEAD; commits since the 08-07 third-invocation entry are all
  docs-only routine-log entries, none matching `^vX.Y.Z`), 0 open issues in
  audio-dl / audio-dl-internal. project-tzu#13 unchanged since 07-31 (same
  head SHA `1405d307...`, same stale base `worktree-verified-undo-v1.1`,
  `mergeable_state: clean`, same 2-file/+86-10 diff, one comment, still
  self-authored so never qualifies as "unaddressed") — left alone, no new
  comment. Applied the standing project-tzu-scoping judgment once more
  (Steps 1-4/4.8 yes; 4.5/4.6 no; 4.7 yes, moot — nothing reached
  APPROVED+green in any repo this run). No new findings — items 1-11
  remain the complete, stable backlog; fifteen runs of confirmation
  without incorporation into the live routine prompt. Nothing-to-do run —
  no Slack/push notification sent, per the empty-run silence policy.
- **2026-08-08 (second invocation):** Sixteenth consecutive nothing-to-do
  run, same steady state as the first 08-08 entry — 0 open PRs in audio-dl
  / audio-dl-internal, 0 untagged release commits (`v2.5.0` still matches
  the latest release-commit subject on `main` HEAD; the only commit since
  the first 08-08 entry is that entry's own doc commit, docs-only, doesn't
  match `^vX.Y.Z`), 0 open issues in audio-dl / audio-dl-internal.
  project-tzu#13 unchanged since 07-31 (same head SHA `1405d307...`, same
  stale base `worktree-verified-undo-v1.1`, `mergeable_state: clean`, same
  2-file/+86-10 diff, one comment, still self-authored so never qualifies
  as "unaddressed") — left alone, no new comment. `CronList` confirms no
  editable session-local job, as every prior run has found. Applied the
  standing project-tzu-scoping judgment once more (Steps 1-4/4.8 yes;
  4.5/4.6 no; 4.7 yes, moot — nothing reached APPROVED+green in any repo
  this run). No new findings — items 1-11 remain the complete, stable
  backlog; sixteen runs of confirmation without incorporation into the
  live routine prompt. Nothing-to-do run — no Slack/push notification
  sent, per the empty-run silence policy.
