...
> **Note:** The advisor tool is in beta. Include the beta header `advisor-tool-2026-03-01` in your requests.
...
1. The executor emits a `server_tool_use` block with `name: "advisor"` and an empty `input`. The executor signals timing, and the server supplies context.
...
Claude Fable 5 and Claude Mythos 5 advisors return `advisor_redacted_result`. The other advisor models in the compatibility table return `advisor_result`.
...
To force a consult on a specific request instead of nudging, set `tool_choice` to `{"type": "tool", "name": "advisor"}`, subject to the constraints in Forcing tool use. Forcing tool use cannot be combined with extended thinking: The API returns a `400 invalid_request_error` if you enable both.
...
The top-level `max_tokens` applies to executor output only. It does not bound advisor sub-inference tokens. To cap advisor output directly, set `max_tokens` on the tool definition. The advisor's tokens also do not draw from any task budget applied to the executor.

Priority Tier applies to each model independently. A Priority Tier commitment on the executor model does not extend to the advisor. Advisor calls run at Priority Tier only if your organization also holds a commitment on the advisor model.
...
**Caveat:** On an internal browse-comprehension benchmark (n = 1,266), a close variant of this block cost roughly 4 percentage points of accuracy relative to the built-in default. If your workload mixes coding with substantial lookup or retrieval, stay with the suggested blocks, or gate the swap on a workload-type signal you already compute.
...
> **Note:** In Anthropic's testing this line also increased how often the executor consults the advisor, but the net effect was still lower total cost (more consults, each shorter).

Pair this approach with the timing guidance in the suggested system prompt for coding tasks (or the alternative Haiku block if you swapped it in) for the strongest cost-versus-quality tradeoff. For a hard ceiling rather than a soft request, see [Capping advisor output](#capping-advisor-output).
...
Hard reasoning tasks elicit substantially longer advisor output than the typical 1,400 to 1,800 tokens quoted earlier for lighter workloads. Use this table to size the savings ratio, not as a universal baseline for advisor output.
...
Compared with the prompt-based approach, `max_tokens` is a hard ceiling rather than a soft request. Use `max_tokens` when you need a guaranteed bound for cost or latency. Use the prompt-based approach (or both together) when you want to bias toward brevity without risking a mid-thought cut.
